(function($){
	$.fn.ecuiIconSelect = function(options) {
		...
		function init() {
			$container = $('<div/>', {'class':'ecui-icon-select-container'})
				.insertAfter($input);
				
			addIconBoxes();
			$container.on('click', '.ecui-icon-box', doOnBoxSelected);
			
			//make the first box selected
			var savedIcon = $input.attr('value'),
				$box;
				
			if(savedIcon){
				$box = $container.find('.ecui-icon-box').filter(function(){
					return $(this).data('icon') == savedIcon;
				}).eq(0);
			}else{
				$box = $container.find('.ecui-icon-box:first');
			}
			doOnBoxSelected.call($box);
		}
		
		function addIconBoxes(){
			//add an empty box
			$container.append($('<i/>', {'class':'ecui-icon-box'}).data('icon', 'none'));
			
			for(var i in o.icons){
				var icon = o.icons[i];
				$container.append($('<i/>', {'class':'ecui-icon-box fa '+icon})
					.data('icon', icon)
				);
			}
		}
...
jQuery(document).ready(function($){
	...
	$('.ecui-icon-select').each(function(){
		$(this).ecuiIconSelect();
	});
});